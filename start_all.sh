#!/bin/bash
# Скрипт для запуска сервера и бота одновременно

set -e

# Переход в директорию скрипта
cd "$(dirname "$0")"

# Активация виртуального окружения
if [ -d ".venv" ]; then
    source .venv/bin/activate
else
    echo "❌ Виртуальное окружение не найдено. Запустите ./deploy.sh"
    exit 1
fi

# Загрузка переменных окружения
if [ -f ".env" ]; then
    export $(cat .env | grep -v '^#' | xargs)
else
    echo "⚠️  .env файл не найден"
fi

echo "🚀 Запуск сервера и бота..."

# Запуск сервера в фоне
echo "📡 Запуск сервера..."
nohup uvicorn server.main:app --host 0.0.0.0 --port ${PORT:-8001} --log-level ${LOG_LEVEL:-info} > server.log 2>&1 &
SERVER_PID=$!
echo "   Сервер запущен (PID: $SERVER_PID)"

# Небольшая задержка для запуска сервера
sleep 2

# Проверка, что сервер запустился
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo "❌ Ошибка запуска сервера. Проверьте server.log"
    exit 1
fi

# Запуск бота в фоне (как модуль)
echo "🤖 Запуск бота..."
nohup python -m bot.bot > bot.log 2>&1 &
BOT_PID=$!
echo "   Бот запущен (PID: $BOT_PID)"

# Сохранение PID в файл для удобной остановки
echo $SERVER_PID > server.pid
echo $BOT_PID > bot.pid

echo ""
echo "✅ Сервер и бот запущены!"
echo ""
echo "📊 Просмотр логов:"
echo "   tail -f server.log  # Логи сервера"
echo "   tail -f bot.log    # Логи бота"
echo ""
echo "🛑 Остановка:"
echo "   ./stop_all.sh      # Остановить оба процесса"
echo "   kill $SERVER_PID   # Остановить только сервер"
echo "   kill $BOT_PID      # Остановить только бота"
echo ""
echo "📋 Проверка процессов:"
echo "   ps aux | grep uvicorn"
echo "   ps aux | grep bot.py"
