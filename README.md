...
## 🤖 Версия 2.0: Выбор модели классификации

### Доступные модели

#### Обученные модели (на нашем датасете)
| Модель | Описание | Macro-F1 |
|--------|----------|----------|
| Logistic Regression | Быстрая классическая модель | X.XX |
| SVM | Support Vector Machine | X.XX |
| Naive Bayes | Вероятностная модель | X.XX |
| Random Forest | Ансамбль деревьев | X.XX |
| BiLSTM | Двунаправленная LSTM | X.XX |
| CNN | Свёрточная нейросеть | X.XX |
| BERT | Fine-tuned rubert-tiny2 | X.XX |
| Ensemble | Взвешенный ансамбль | X.XX |

#### Pretrained модели (HuggingFace)
| Модель | Описание |
|--------|----------|
| XLM-R Formality | Multilingual классификатор формальности |
| RuBERT Sentiment | Sentiment как прокси для стиля |

### Использование в боте

1. `/start` - начало работы
2. `/model` - выбор модели классификации
3. `/info` - информация о текущей модели
4. Отправьте голосовое или текст для классификации
...
curl -X POST http://localhost:8000/predict_text \
  -F "text=Уважаемый коллега, прошу рассмотреть" \
  -F "model=ensemble"
...
## Telegram-бот
Принимает голосовые/аудиофайлы, отправляет на `/predict`, поддерживает выбор модели. Возвращает класс, уверенность, длительность, число слов и превью текста.
